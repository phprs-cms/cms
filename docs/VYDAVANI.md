...
3. `dist/phprs-X.Y.Z.zip` nahrajte do GitHub Releases, `dist/aktualizace.json` na `https://phprs.eu/aktualizace.json`.
...
aktualizaci nepřijmou, kompromitovanému klíči věří – útočník ale k útoku potřebuje ještě ovládnout `phprs.eu/aktualizace.json`.
