...
You turn on dark mode for readers in the same place – the site then follows the setting of their device.

## 2. Create sections

**Content → Sections.** Every article belongs to exactly one section; an article cannot be saved without one. The installation created the section *News* (with sample content, the five sections of a fictional daily); rename it and add more.
Sections can have subsections. Their order in the navigation is set by the **Order** field.
...
**Appearance → Blocks and layout.** Your site opens in editing mode. You add blocks – a list of sections, most read articles, search, custom text and others – with the **+ Add block** button right where they should be, and you drag them with the mouse. At the top you choose the layout: three columns, two, one or full width.
...
- **Two-factor sign-in** for all administrators (**My account**).
- **Settings → System status** – go through the items with an exclamation mark. Each one says what is wrong and where to fix it.
- **Off-site backups** (**Settings → Backups and updates**) – a backup on the same server as the site will not help if you lose the hosting.
- **More people in the newsroom** – **Administration → Users**. An author writes and submits for review, an editor publishes, an administrator manages the whole site.
...
The keyboard shortcut **Ctrl+K** (**⌘K** on a Mac) opens the command palette: type the name of a section, action or article and press Enter. It also works without accents.
