...
final class Demo
{
    private const string SLOZKA = PHPRS_SYSTEM . '/demo';
    ...
    public static function nahraj(Db $db, Settings $nastaveni, string $jazyk, ?int $autor): void
    {
        ...
        foreach ($data['clanky'] as $klic => $spolecne) {
            ...
            \PhpRS\Admin\Moduly\Galerie::zapisPouziti($db, $idc, $obrazek);
            $zaznam['clanky'][] = $idc;
        }
    ...
    private static function nahrajObrazek(Db $db, string $soubor, string $nazev, ?int $autor, array &$zaznam): string
    {
        ...
        if (!is_file($zdroj) || (!is_dir(PHPRS_ROOT . '/' . $slozka) && !@mkdir(PHPRS_ROOT . '/' . $slozka, 0775, true)) || !@copy($zdroj, PHPRS_ROOT . '/' . $cil)) {
            return '';
        }
        ...
        if ($maly !== false && imagejpeg($maly, PHPRS_ROOT . '/' . $slozka . '/demo-' . basename($soubor, '.jpg') . '-nahled.jpg', 85)) {
