...
/**
 * Export celého webu do jednoho archivu – aby obsah nikdy nezůstal v phpRS zamčený.
 *
 ...
 * Není to záloha pro obnovu phpRS (tou je záloha databáze), ale přenosný otevřený formát.
 */
final class ExportWebu
{
    ...
    public static function vytvor(Db $db, Settings $nastaveni): array
    {
        ...
        if ($duvod === '') {
            ...
            foreach (array_keys($soubory) as $cesta) {
                $zip->addFile(PHPRS_ROOT . '/' . $cesta, $cesta);
    ...
    private static function zapisObsah(Db $db, Settings $nastaveni, string $cesta): void
    {
        ...
        fwrite($f, '{"format":"phprs-export","verze_formatu":1,"phprs":' . self::json(PHPRS_VERSION) . ',"vytvoreno":' . self::json(date('c')) . ',"nastaveni":' . self::json(self::nastaveni($db)));
    ...
    {
        foreach (self::postupne($db, "SELECT c.*, {$autori} FROM {clanky} c WHERE c.idc > ? ORDER BY c.idc LIMIT 100", 'idc') as $c) {
    ...
    private static function media(): array
    {
        ...
        $pruchod = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator(PHPRS_ROOT . '/media', \FilesystemIterator::SKIP_DOTS));
        foreach ($pruchod as $soubor) {
            $cesta = 'media/' . ltrim(str_replace('\\', '/', substr($soubor->getPathname(), strlen(PHPRS_ROOT . '/media'))), '/');
    ...
    {
        return "Export webu z redakčního systému phpRS " . PHPRS_VERSION . " (" . date('j. n. Y H:i') . ")\n"
            . "=====================================================================\n\n"
            ...
            . "format, verze_formatu, phprs, vytvoreno – hlavička\n"
