...
{
    private const string SEZNAM = PHPRS_SYSTEM . '/soubory.json';

    /** @return array{stav:string, info:string, zmenene:list<string>, chybi:list<string>, navic:list<string>} */
    public static function kontrola(string $klicSoubor = PHPRS_SYSTEM . '/aktualizace.pub'): array
    {
        ...
        $navic = [];
        $kandidati = glob(PHPRS_ROOT . '/*.php') ?: [];
        $strom = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator(PHPRS_SYSTEM, \FilesystemIterator::SKIP_DOTS));
        foreach ($strom as $f) {
        ...
        foreach ($kandidati as $soubor) {
            $cesta = ltrim(str_replace('\\', '/', substr($soubor, strlen(PHPRS_ROOT))), '/');
    ...
    public static function kPodpisu(string $verze, array $soubory): string
    {
        ...
        return 'phprs-soubory|' . $verze . '|' . hash('sha256', (string) json_encode($soubory, JSON_UNESCAPED_SLASHES));
    }
}
