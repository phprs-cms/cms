...
final class Rozesilka
{
    ...
    public static function davka(App $app, int $idn): bool
    {
        ...
        $davka = $db->all('SELECT * FROM {odberatele} WHERE potvrzen = 1 AND ido > ? ORDER BY ido LIMIT ' . self::DAVKA, [(int) $v['posledni']]);
        foreach ($davka as $o) {
    ...
    private static function automat(App $app): void
    {
        ...
        $db = $app->db();
        $od = (string) ($db->value('SELECT MAX(vytvoreno) FROM {newsletter}') ?? date('Y-m-d H:i:s', time() - ($rezim === 'tydne' ? 7 : 1) * 86400));
        $od = max($od, date('Y-m-d H:i:s', time() - 14 * 86400));
        $ids = array_column($db->all("SELECT idc FROM {clanky} WHERE visible = 1 AND datum <= NOW() AND datum > ? AND typ_clanku = 1 AND noindex = 0 AND jazyk = '' ORDER BY priority DESC, visit DESC, datum DESC LIMIT 8", [$od]), 'idc');
        if ($ids === [] || (int) $db->value('SELECT COUNT(*) FROM {odberatele} WHERE potvrzen = 1') === 0) {
            return; // nic nového, nebo není komu psát
        }
        $prvni = (string) $db->value('SELECT titulek FROM {clanky} WHERE idc = ?', [$ids[0]]);
        $db->insert('newsletter', [
            'predmet' => mb_substr($prvni . (count($ids) > 1 ? ' a další – ' : ' – ') . $s->get('nazev_webu'), 0, 200), 'uvod' => $s->get('newsletter_uvod'),
            'clanky' => implode(',', $ids), 'vytvoreno' => date('Y-m-d H:i:s'), 'auto' => 1, 'odeslat_v' => date('Y-m-d H:i:s'),
        ]);
    }
