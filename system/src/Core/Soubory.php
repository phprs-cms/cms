...
final class Soubory
{
    ...
    public static function uloz(array $file): array
    {
        ...
        return ['obr_poloha' => $cil, 'obr_width' => 0, 'obr_height' => 0, 'obr_vel' => (int) filesize(PHPRS_ROOT . '/' . $cil),
            'nahl_poloha' => '', 'nahl_width' => 0, 'nahl_height' => 0, 'nazev' => mb_substr($nazev, 0, 150)];
    }

    public static function smaz(string $cesta): void
    {
        if (preg_match('#^media/\d{4}/\d{2}/[a-z0-9-]+\.(' . implode('|', self::PRIPONY) . ')$#', $cesta) && is_file(PHPRS_ROOT . '/' . $cesta)) {
