 ...
 *
 * Platba i správa karty probíhá na stránkách Stripe (Checkout a Customer Portal); phpRS nikdy nevidí číslo karty.
...
class Stripe
{
    ...
    public static function zaloguj(string $zprava): void
    {
        ...
        @file_put_contents(PHPRS_ROOT . '/storage/log/chyby.log', sprintf("[%s] Stripe: %s\n", date('c'), mb_substr($zprava, 0, 500)), FILE_APPEND | LOCK_EX);
    }
    ...
    private function adresa(array $odpoved): string
    {
        ...
        if (!preg_match('#^https://[^\s"<>]+$#', $adresa) && !(defined('PHPRS_STRIPE_URL') && preg_match('#^http://[^\s"<>]+$#', $adresa))) {
    ...
    protected function zavolej(string $metoda, string $cesta, array $pole = []): array
    {
        ...
        $adresa = rtrim(defined('PHPRS_STRIPE_URL') ? (string) constant('PHPRS_STRIPE_URL') : 'https://api.stripe.com', '/') . $cesta;
        $hlavicky = ['Authorization: Bearer ' . $klic, 'Stripe-Version: ' . self::VERZE_API, 'Content-Type: application/x-www-form-urlencoded', 'User-Agent: phpRS/' . PHPRS_VERSION];
