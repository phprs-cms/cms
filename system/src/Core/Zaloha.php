...
final class Zaloha
{
    public const string SLOZKA = PHPRS_ROOT . '/storage/zalohy';
    ...
    public static function vytvor(Db $db, string $duvod = 'rucni'): string
    {
        ...
        $soubor = 'phprs-' . date('Ymd-His') . '-' . preg_replace('/[^a-z0-9]/', '', $duvod) . '-' . bin2hex(random_bytes(4)) . '.sql' . ($gz ? '.gz' : '');
        $cesta = self::SLOZKA . '/' . $soubor;
        $f = $gz ? gzopen($cesta, 'wb6') : fopen($cesta, 'wb');
        $zapis = fn (string $s) => $gz ? gzwrite($f, $s) : fwrite($f, $s);

        $pdo = $db->pdo();
        $zapis("-- phpRS " . PHPRS_VERSION . " - záloha databáze " . date('c') . "\nSET NAMES utf8mb4;\nSET FOREIGN_KEY_CHECKS = 0;\n\n");
    ...
    {
        $zalohy = [];
        foreach (glob(self::SLOZKA . '/phprs-*.sql*') ?: [] as $cesta) {
    ...
    {
        return preg_match('/^phprs-[0-9a-z-]+\.sql(\.gz)?$/', $soubor) && is_file(self::SLOZKA . '/' . $soubor) ? self::SLOZKA . '/' . $soubor : null;
    }
