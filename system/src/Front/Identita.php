 ...
 *
 * Šablony s identitou počítají přes CSS proměnné --rs-akcent, --rs-pismo-titulky a --rs-pismo-text:
 * ve svém style.css je použijí s vlastní výchozí hodnotou, např. --akcent: var(--rs-akcent, #326891).
...
final class Identita
{
    ...
    public static function hlava(Settings $web, string $zaklad): string
    {
        ...
        if (preg_match('/^#[0-9a-f]{6}$/i', $web->get('brand_akcent'))) {
            $promenne[] = '--rs-akcent: ' . $web->get('brand_akcent');
        }
        foreach (['brand_pismo_titulky' => [self::PISMA_TITULKU, '--rs-pismo-titulky'], 'brand_pismo_text' => [self::PISMA_TEXTU, '--rs-pismo-text']] as $klic => [$pisma, $promenna]) {
