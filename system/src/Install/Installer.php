...
final class Installer
{
    ...
    public function handle(): Response
    {
        ...
        \PhpRS\Core\Jazyk::nastav($this->jazyk, 'install-');
    ...
    private function instaluj(array $d, string $heslo, string $heslo2): array
    {
        ...
        if (!preg_match('/^[a-z][a-z0-9_]{0,15}$/', $d['db_prefix'])) {
            $chyby['db_prefix'] = t('Předpona: malá písmena, číslice a podtržítko, nejvýše 16 znaků (např. rs_).');
        }
    ...
    private function stranka(string $sablona, array $data): Response
    {
        return Response::html($this->view->render('install/' . $sablona, $data + [
            'base' => $this->request->basePath(), 'jazyk' => \PhpRS\Core\Jazyk::kod(),
            'jazyky' => array_intersect_key(\PhpRS\Core\Jazyk::ADMINISTRACE, self::PASMA),
        ]));
    }
}
