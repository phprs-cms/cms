...
use PhpRS\Admin\Moduly\Galerie;
...
final class Nastroje
{
    ...
    public function seznam(): array
    {
        ...
        $clanek = [
            ...
            'obrazek' => $text('Adresa hlavního obrázku (z nástroje seznam_medii)'), 'datum' => $text('Datum vydání RRRR-MM-DD HH:MM; budoucí = naplánování'),
        ...
        switch ($nazev) {
            case 'info_o_webu':
                return [
                    ...
                    'sablona' => $web->get('layout'), 'rozvrzeni' => $web->get('rozvrzeni'), 'verze_phprs' => PHPRS_VERSION,
                    'clanku_vydanych' => (int) $db->value('SELECT COUNT(*) FROM {clanky} WHERE visible = 1 AND datum <= NOW()'),
                    'uzivatel' => $auth->user()['user'], 'role' => \PhpRS\Core\Auth::TYPY[(int) $auth->user()['admin']], 'smi_vydavat' => $auth->smiVydavat(),
                ];
            ...
            case 'nacti_clanek':
                ...
                return array_intersect_key($c, array_flip(['idc', 'titulek', 'seo_link', 'uvod', 'text', 'obrazek', 'datum', 'visible', 'shrnuti', 'faq', 'seo_titulek', 'seo_popis', 'visit']))
            ...
            case 'vytvor_sablonu':
                ...
                mkdir(PHPRS_ROOT . '/layout/' . $nova, 0775);
                foreach (glob(PHPRS_ROOT . '/layout/' . $podle . '/*.{php,css}', GLOB_BRACE) ?: [] as $soubor) {
                    $obsah = (string) file_get_contents($soubor);
                    // zkopírovaná šablona musí odkazovat na vlastní style.css
                    file_put_contents(PHPRS_ROOT . '/layout/' . $nova . '/' . basename($soubor), str_replace("layout/{$podle}/", "layout/{$nova}/", $obsah));
                }
                file_put_contents(PHPRS_ROOT . '/layout/' . $nova . '/info.php', "<?php\n\nreturn " . var_export([
    ...
    private function ulozClanek(?array $puvodni, array $a): array
    {
        ...
        $data = [];
        foreach (['titulek' => 255, 'uvod' => 0, 'text' => 0, 'shrnuti' => 0, 'seo_popis' => 320, 'obrazek' => 255] as $pole => $max) {
    ...
    {
        return array_map(basename(...), glob(PHPRS_ROOT . '/layout/' . $slozka . '/*.{php,css}', GLOB_BRACE) ?: []);
    }
    ...
    private function souborSablony(string $slozka, string $soubor, bool $musiExistovat): string
    {
        ...
        $cesta = PHPRS_ROOT . '/layout/' . $slozka . '/' . $soubor;
