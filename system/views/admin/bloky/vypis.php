...
<?php foreach ($bloky[$klic] as $b): ?>
			<article class="blok-karta<?= $b['sys_funkce'] !== '' ? ' blok-karta-sys' : '' ?><?= $b['zobrazit'] ? '' : ' blok-skryty' ?>" draggable="true" data-idb="<?= (int) $b['idb'] ?>">
				...
				<span><?= $b['sys_funkce'] !== '' ? 'systémový · ' . e(Bloky::SYSTEMOVE[$b['sys_funkce']] ?? $b['sys_funkce']) : 'vlastní HTML' ?><?= $b['zobrazit'] ? ((int) $b['zobrazit_kde'] !== 0 ? ' · ' . e(Bloky::KDE[(int) $b['zobrazit_kde']]) : '') : ' · <b>skrytý</b>' ?></span>
