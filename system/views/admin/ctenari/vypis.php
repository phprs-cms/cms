...
<p class="smltxt"><?= e(t($stripe
	...
	: 'Článek zamknete v jeho editoru volbou „Kdo smí číst“. Předplatné se zapisuje ručně – například po přijetí platby na účet.')) ?> <?= PhpRS\Core\Napoveda::odkaz('ctenari-a-prijmy/platby-stripe', 'Platby přes Stripe') ?></p>
<?php if ($ctenari === []): ?>
...
<tbody>
<?php foreach ($ctenari as $c): $plati = $c['predplatne_do'] !== null && $c['predplatne_do'] >= date('Y-m-d'); $bezi = PhpRS\Front\Ctenari::beziStripe($c); ?>
<tr>
