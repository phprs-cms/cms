...
<?php if ((int) $a['idu'] !== $app->auth()->id()): ?>
		/ <form class="vradku" method="post" action="<?= e($modul->url('smaz')) ?>" data-potvrdit="<?= e(t('Opravdu smazat autora?')) ?>"><?= $csrf ?><input type="hidden" name="idu" value="<?= (int) $a['idu'] ?>"><button class="navigace" type="submit"><?= e(t('Smaž')) ?></button></form>
<?php endif ?>
	</td>
</tr>
<?php endforeach ?>
