...
<?php if ($k['reakce_na'] === null): ?>
			<button type="button" class="komentar-reagovat" data-reagovat="<?= (int) $k['idk'] ?>" data-jmeno="<?= e($k['od']) ?>">Reagovat</button>
<?php endif ?>
		</p>
...
<?php else: ?>
	<p>Zatím tu žádný komentář není. Buďte první.</p>
<?php endif ?>
	<form class="komentar-formular" method="post" action="<?= e($akce) ?>">
		...
		<p class="komentar-reakce-info" hidden>Reagujete na komentář: <strong></strong> <button type="button" data-zrusit-reakci>zrušit</button></p>
		<p><label for="kom-od">Jméno</label><input type="text" id="kom-od" name="od" maxlength="60" required autocomplete="name"></p>
		<p><label for="kom-mail">E-mail <small>(nepovinný, nezveřejní se)</small></label><input type="email" id="kom-mail" name="od_mail" maxlength="190" autocomplete="email"></p>
		<p><label for="kom-obsah">Komentář</label><textarea id="kom-obsah" name="obsah" rows="5" maxlength="5000" required></textarea></p>
		<p><button type="submit">Odeslat komentář</button></p>
	</form>
</section>
<script>
