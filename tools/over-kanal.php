<?php
/**
 * Ověří kanál aktualizací tak, jak ho vidí instalace phpRS: co právě visí na webu projektu, musí být podepsané
...
use PhpRS\Core\Podpis;

$adresa = $argv[1] ?? 'https://phprs.eu/aktualizace.json';
$klice = dirname(__DIR__) . '/system/aktualizace.pub';
$chyby = [];
$stahni = static function (string $url): string {
    $data = @file_get_contents($url, false, stream_context_create(['http' => ['timeout' => 60, 'follow_location' => 1, 'max_redirects' => 5, 'header' => "User-Agent: phpRS-kontrola\r\n"]]));
    if ($data === false) {
...
try {
    ...
    $zip = tempnam(sys_get_temp_dir(), 'phprs');
