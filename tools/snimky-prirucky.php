 ...
 *
 *   PHPRS_SNIMKY_COOKIE="phprs3=<session přihlášeného správce>" php tools/snimky-prirucky.php [jazyk…] [--jen=nazev,nazev]
 *
 * Potřebuje: Google Chrome, cwebp, běžící instanci (výchozí http://localhost:8080, jinak PHPRS_SNIMKY_CIL) a config.php
...
$cookie = (string) getenv('PHPRS_SNIMKY_COOKIE');
$cil = rtrim((string) (getenv('PHPRS_SNIMKY_CIL') ?: 'http://localhost:8080'), '/');
if (!preg_match('/^phprs3=[a-f0-9]{16,}$/', $cookie) || !is_file(CHROME)) {
    fwrite(STDERR, "Chybí PHPRS_SNIMKY_COOKIE (phprs3=…) nebo Google Chrome.\n");
    exit(1);
}
...
$config = require PHPRS_ROOT . '/config.php';
$db = PhpRS\Core\Db::fromConfig($config['db']);
// účet, kterému patří předaná session (jazyk administrace je volba účtu); výchozí jméno jde změnit přes PHPRS_SNIMKY_UCET
$ucet = $db->one('SELECT idu, jazyk FROM {user} WHERE user = ?', [(string) (getenv('PHPRS_SNIMKY_UCET') ?: 'admin')]);
if ($ucet === null) {
    fwrite(STDERR, "Nenašel jsem účet správce (PHPRS_SNIMKY_UCET).\n");
    exit(1);
}

$proxy = proc_open(['php', '-S', '127.0.0.1:' . PORT, __DIR__ . '/snimky-proxy.php'], [1 => ['file', '/dev/null', 'w'], 2 => ['file', '/dev/null', 'w']], $roury, null,
    ['PHPRS_SNIMKY_CIL' => $cil, 'PHPRS_SNIMKY_COOKIE' => $cookie, 'PATH' => (string) getenv('PATH')]);
usleep(800000);
$tmp = sys_get_temp_dir() . '/phprs-snimky-' . getmypid();
mkdir($tmp);
$chyb = 0;
try {
    foreach ($jazyky as $jazyk) {
        $db->update('user', ['jazyk' => $jazyk], ['idu' => $ucet['idu']]);
        $slozka = PHPRS_ROOT . "/docs/prirucka/$jazyk/obrazky";
