 ...
 *
 *   php tools/vydani.php 3.0.1 --url=https://github.com/phprs-cms/cms/releases/download/v3.0.1/phprs-3.0.1.zip \
...
if (str_starts_with($verze, '--novy-klic=')) {
    ...
    file_put_contents($pub, rtrim((string) @file_get_contents($pub)) . "\n" . base64_encode($pk) . ' ' . $jmeno . ' ' . date('Y-m-d') . ' id=' . PhpRS\Core\Podpis::id($pk) . "\n");
    file_put_contents($pub, ltrim((string) file_get_contents($pub)));
    exit("Nový klíč „{$jmeno}“ (id " . PhpRS\Core\Podpis::id($pk) . ") je v {$cil}.\n"
...
$idKlice = PhpRS\Core\Podpis::id(sodium_crypto_sign_publickey_from_secretkey($sk));
if (!isset(PhpRS\Core\Podpis::klice($verejny)[$idKlice])) {
...
$manifest = [
    ...
    'podpis' => base64_encode(sodium_crypto_sign_detached(PhpRS\Core\Podpis::zpravaBalicku($verze, $sha, $volby['bezpecnostni']), $sk)),
